 ...
 * side panel. Neither `setPopup` nor the panel's own behaviour survives a browser restart, so the
 * choice lives in storage and the worker applies it — on install, on startup, and whenever it
 * changes here. What that means to each browser is `./panel.ts`; they do not agree.
 *
 ...
 */
import { applyPanelBehavior } from './panel';
...
export async function applySurface(surface: Surface) {
  await applyPanelBehavior(surface === 'panel');
