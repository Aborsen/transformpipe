...
**Il titolo se ne va con lui.** L'unico pezzo di metadato che ogni formato di output vuole è il titolo, ed eliminarlo lo cancella. Un file HTML senza `<title>` mostra il nome del file nella scheda del browser, cosa che qualcuno vede nella sua barra delle schede e nei suoi segnalibri. Un documento chiamato `final-v3.html` seduto in una scheda è una piccola indignità che una modifica di due righe evita: interpreta il blocco, tieni `title`, mettilo nella head. Lo stesso vale per la descrizione, che è ciò che un client di chat legge per costruire l'anteprima di un link.
